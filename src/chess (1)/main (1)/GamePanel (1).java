...
private void copyPieces(ArrayList<Piece> Beginning, ArrayList<Piece> Destination) {
    ...
    public void run(){// override runnab
    ...
    private void update (){// handle updating stuff piece position number of the piece.
    ...
    private void simulate(){//拖拉程式码 d
        ...
        if(activePiece.canMove(activePiece.column, activePiece.row)) {//檢查落點是否可走。
            ...
            if(PawnCanFusionWithRook()){
                FusionPawnAndCar = true;
            }
            else{
                FusionPawnAndCar =false;
            }
    ...
    private boolean canPromote(){
        ...
        if(activePiece.type == Type.Pawn){
            if((currentColor == "white" && activePiece.row == 0) ||(currentColor == "black" && activePiece.row == 7)){
                ...
                promoPieces.add(new Knight(currentColor,9,5));
                return true;
            }
        }
        return false;
